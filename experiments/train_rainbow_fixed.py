...
class ThresholdBasedProactiveSDN(gym.Env):
    ...
    def _initialize_mappings(self):
        ...
        for switch_id in range(self.num_switches):
            controller_idx = switch_id % len(self.active_slaves)
            controller_id = list(self.active_slaves)[controller_idx]
    ...
    def _execute_action(self, action: int) -> Tuple[str, bool]:
        ...
        if action == 0:
            ...
            if parked_idx < len(list(self.parked_slaves)):
                parked_id = list(self.parked_slaves)[parked_idx]
