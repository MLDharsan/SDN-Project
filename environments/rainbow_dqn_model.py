...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        # Get samples - convert to numpy first to avoid warning
...
class RainbowDQN:
    ...
    def train_step(self):
        ...
        # Cross-entropy loss
        loss_per_sample = -(target_q_dist * q_dist.clamp(min=1e-5, max=1).log()).sum(dim=1)
        
        # Importance sampling weights
        weighted_loss = (loss_per_sample * weights).mean()
        
        # Update priorities (use per-sample loss, not mean)
        priorities = loss_per_sample.detach().cpu().numpy()
